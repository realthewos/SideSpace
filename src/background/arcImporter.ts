import type { ArcExportData, ArcSpace, ArcContainer, ArcItem, ArcImportResult } from '../shared/types/arc';
import { StorageManager } from './storageManager';

export class ArcImporter {
  ...
  async importFromRawData(rawData: any): Promise<ArcImportResult> {
    ...
    try {
      ...
      if (this.isStorableSidebar(rawData)) {
        ...
        throw new Error('Unrecognized file format. Expected Arc StorableSidebar.json or Chromium Bookmarks format.');
      }

      // Import the parsed data
      await this.importData(arcData, result);

      result.success = result.spacesImported > 0;
    } catch (error: any) {
  ...
  private isStorableSidebar(data: any): boolean {
    return data && (data.spaces || data.sidebar || data.containers || data.items);
  }
  ...
  private parseStorableSidebar(data: any): ArcExportData {
    ...
    const spaceList = data.spaces || (data.sidebar && data.sidebar.spaces) || [data];

    for (const spaceData of spaceList) {
      const space: ArcSpace = {
        id: spaceData.id || `arc-space-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
  ...
  private extractContainers(spaceData: any): ArcContainer[] {
    ...
    if (Array.isArray(items)) {
      if (items.length > 0 && (items[0].container || items[0].parentId)) {
        ...
        for (const item of items) {
          ...
          containerMap.get(containerId)!.push({
            ...
            url: item.url || (item.data && item.data.url) || '',
  ...
  private parseChromiumBookmarks(data: any): ArcExportData {
    ...
    const processFolder = (folder: any): ArcContainer[] => {
      const containers: ArcContainer[] = [];
      ...
      for (const child of folder.children || []) {
        if (child.type === 'url') {
          items.push({
            id: `bm-${child.id || Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
          ...
          const subContainers = processFolder(child);
    ...
    if (data.roots.bookmark_bar) {
      const containers = processFolder(data.roots.bookmark_bar);
    ...
    if (data.roots.other && data.roots.other.children && data.roots.other.children.length > 0) {
      const containers = processFolder(data.roots.other);
      if (containers.length > 0) {
        spaces.push({
          id: 'imported-other',
